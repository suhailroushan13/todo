...
async function verifyEmailPassword() {
  try {
    ...
    let userData = fileData.find((ele) => ele.email === email);

    if (userData) {
      const pwdValid = await bcrypt.compare(password, userData.password);
      if (!pwdValid) {
        console.log(color.red_bt("\nInvalid Credentials!\n"));
        return;
      }
      console.log(
        color.green_bt("\nUser Login Successfull. Copy the Token below : \n")
      );
      let payload = {
        email: userData.email,
        mobile: userData.mobile,
